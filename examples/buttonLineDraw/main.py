...
def main():
   ...
   while True:
      ...
      if GPIO.input(21) == 0: # Top
         ...
         LCD.LCD_ShowImage(image,0,0)
         time.sleep (0.5)
      elif GPIO.input(6) == 0: # up button, pong      
         image = Image.new("RGB", (LCD.width, LCD.height), "BLACK")
         draw = ImageDraw.Draw(image)         
         draw.line((64,1,64,127),fill="WHITE",width=5)                  
         draw.line((1,20,1,55),fill="WHITE",width=5)                  
         draw.line((127,90,127,125),fill="WHITE",width=5)                  
         #font = ImageFont.truetype('/usr/share/fonts/truetype/freefont/FreeMonoBold.ttf', 32)
         draw.text((33, 90), '0', fill = "White")
         draw.text((90, 90), '1', fill = "White")
         LCD.LCD_ShowImage(image,0,0); 
         time.sleep (0.5)
      elif GPIO.input(19) == 0: # down button, tic-tac-toe
         image = Image.new("RGB", (LCD.width, LCD.height), "WHITE")
         draw = ImageDraw.Draw(image)         
         draw.line((43,1,43,127),fill="BLACK",width=5)                  
         draw.line((86,1,86,127),fill="BLACK",width=5)                  
         draw.line((1,43,127,43),fill="BLACK",width=5)                  
         draw.line((1,86,127,86),fill="BLACK",width=5)   
         image.show()         
         LCD.LCD_ShowImage(image,0,0);
         time.sleep (0.5)
         ...
         draw = ImageDraw.Draw(image)
         print "***draw line"
         draw.line((1,1,120,120),fill="BLUE",width=5)         
